...
To use $\mathcal{KL}$ to interact with a knowledge base, \textcite{Lokb} defines two operators on epistemic states: $\operatorname{ASK}$ and $\operatorname{TELL}$. Informally, $\operatorname{ASK}$ is used to determine if a sentence is known to a knowledge base whereas $\operatorname{TELL}$ is used to add a sentence to the knowledge base. Since epistemic states are sets of possible worlds, the more sentences that are known reduces the set of possible worlds. For this purpose, an $\operatorname{INITIAL}$ epistemic state is also defined to contain all possible worlds given a finite set of atoms and terms.
...
The $\operatorname{ASK}$ operator determines whether a formula is known to a knowledge base. Formally, given an epistemic state $e$ and any sentence $\alpha$ of $\mathcal{KL}$,
...
When implementing $\operatorname{ASK}$ in Haskell, we must take into account that a domain is implied by "$\models$" so that we can evaluate sentences with quantifiers. As such, we will take a domain as our first argument.
...
The second operation, $\operatorname{TELL}$, asserts that a sentence is true and in doing so reduces which worlds are possible. In practice, $\operatorname{TELL}[\varphi, e]$ filters the epistemic state $e$ to worlds where the sentence $\varphi$ holds. That is,
$$\operatorname{TELL}[\varphi,e] = e \cap \{w \; | \; w \models \varphi \}$$
...
In addition to $\operatorname{ASK}$ and $\operatorname{TELL}$, it is valuable to define an inital epistemic state. $\operatorname{INITIAL}$ is the epistemic state before any $\operatorname{TELL}$ operations. This state contains all possible world states as there is nothing known that eliminates any possible world.
